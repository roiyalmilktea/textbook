...
//----------------------------------------------------------------------
	...
	//IDをセット
	if($mode == 'shinki'){
		$_POST['data']['id'] = getAutoIncreNum($file_path);//AUTO INCREMENT値取得
	}
	
	//期間をセット
	if($_POST['data']['term'] != 0){
		$_POST['data']['term'] = array();
		foreach($_POST['data']['term_y'] as $key => $val){
			$_POST['data']['term'][] = date('Y-m-d H:i:s',strtotime($val.'-'.$_POST['data']['term_m'][$key].'-'.$_POST['data']['term_d'][$key].' '.$_POST['data']['term_h'][$key].':00:00'));
			//期間の日付が正当な日付かどうかチェック
			//if(!checkdate($_POST['data']['term_m'][$key],$_POST['data']['term_d'][$key],$val)) exit('表示期間の日付が存在しない日付です。戻って日付を選択しなおして下さい');
		}
	}
	
	
	//登録データ（カラム順の変更現金。追加の場合は末尾に）
	$dbDataStructure = array('id','up_ymd','public_flag','term','title', 'description','comment','link_url');
	...
	//新規登録時のみ投票カウント用のデータを生成しておく
	if($mode == 'shinki'){
		$countVote = count($_POST['data']['comment']);
		
		$vote_file_path = str_replace('__id__',$data['id'],$vote_file_path);
		$fp = fopen($vote_file_path, "ab") or die("ファイルオープンエラー。data/ディレクトリのパーミッションを変更下さい");
		flock($fp, LOCK_EX);
		$writeData = $data['id'].','.str_repeat('0'.$dataSeparateStr,$countVote);
				...
				if ($_FILES["data"]["size"]['upfile'][$key] < $maxImgSize) {
					...
					if ($imgType == 'image/gif') {
						...
						if(function_exists('exif_read_data') && function_exists('imagerotate')){
							if($exif_datas = @exif_read_data($_FILES['data']['tmp_name']['upfile'][$key])){
